...
新版提供内容相同的两种 macOS app 发布格式：

- **DMG（推荐）：** 打开 DMG，然后将 `Rewritr.app` 拖到 `Applications` 快捷方式。
- **ZIP：** 解压后，将 `Rewritr.app` 移动到稳定位置，例如 `/Applications`。

旧版本可能只提供 ZIP。两种格式包含同一个未经 Apple 签名或公证的应用。请只从本项目的 GitHub Releases 页面下载；如需验证文件，请使用所选文件旁发布的 SHA-256 校验和。

### 安装并允许未签名应用

macOS 可能需要两项授权：因为 Rewritr 未签名且未经公证，需要选择**仍要打开**；为了自动复制和粘贴你选中的文本，需要授予**辅助功能**权限。

1. 将 `Rewritr.app` 安装到最终位置，建议使用 `/Applications`。
2. 尝试打开 Rewritr。如果 macOS 阻止打开，请前往**系统设置 > 隐私与安全性**，向下滚动到**安全性**，点按**仍要打开**，然后确认**打开**。
3. 再次打开 Rewritr，并按提示授予辅助功能权限。
4. 在 Settings 中配置你的 OpenAI-compatible provider。云端 provider 通常需要 API key；本地 provider 可能不需要。

**仍要打开**通常只需确认一次。辅助功能权限与具体的 app bundle 路径绑定，因此之后移动应用可能需要重新授权。
...
当推送 `v*` tag，或手动运行 `Release` workflow 时，GitHub Actions 会构建发布包。workflow 会将 `Rewritr.app` 同时打包成 DMG 和 ZIP，验证两种发布包，并将文件及其 SHA-256 校验和附加到 GitHub Release。
