...
目前發布包是未簽署的 macOS app zip。解壓縮後：

- 將 `Rewritr.app` 移到穩定位置，例如 `/Applications`。
- 開啟 Rewritr。
- 依提示授予輔助使用權限。
- 在 Settings 中設定你的 OpenAI-compatible provider。雲端 provider 通常需要 API key；本機 OpenAI-compatible 模型可能不需要。

macOS 會把輔助使用權限授予到具體的 app bundle 路徑。如果授權後移動了 app，可能需要重新授權。
...
當推送 `v*` tag，或手動執行 `Release` workflow 時，GitHub Actions 會建置發布包。workflow 會將 `Rewritr.app` 打包成 zip，並附加到 GitHub Release，同時產生 SHA-256 校驗和。
